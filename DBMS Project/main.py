...
def cur_auction():
    ...
    try :
        ...
        if con.is_connected():
            ...
            myclient.close()
            return render_template('currentauction.html',proname=record[0][0],proprice=record[0][1],prodes=record[0][2],procat=record[0][3],proimg=record[0][4],proid=record[0][5],initial=record1,endtimes=endtime,allimg=all_img,car=countyar,carno=county)
            # curt=datetime.datetime.now()
            # uct=datetime.datetime(int(pst[:4]),int(pst[5:7]),int(pst[8:10]),int(pst[11:13]),int(pst[14:16]),int(pst[17:19]))
    except Error as Er:
        print(Er)
